...
# 1 multiframe ≈ 0.235 s
# 256 multiframes ≈ 60 seconds
REPEAT_MULTIFRAMES = 256
...
# Placeholder BCCH (we will replace with real spec soon)
def build_bcch():
    bits = [0]*184
    fire = fire_code(bits)
    encoded = gsm_conv_encode(fire)
...
# ============================================================
# FILE WRITER (ATOMIC SWAP)
# ============================================================

def write_active_file(payload):
...
def main():
    print("v17 running — file-based GSM generator")
    FN = 0

    # Initial file
    bits = build_repeating_multiframes(FN)
    payload = bits_to_bytes(bits)
    write_active_file(payload)
